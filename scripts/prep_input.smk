...
# vae_outputs = {
#     'adata_out':os.path.join(config['vae_run_output_dir'], config['cohort']+'_adata_new.h5ad'),
#     'cluster_ranks':os.path.join(config['vae_run_output_dir'], config['cohort']+"_top5_features_across_clusters.tsv"),
...
rule tidy:
    ...
    shell:
        "python prep_scripts/vae_data_prep_sample.py --feature_data {input.feature_data} "
...
rule merge:
    ...
    shell:
        "python prep_scripts/vae_data_prep_concat.py --input_dir {params.input_dir} --output_h5ad {output} "